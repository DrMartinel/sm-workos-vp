...
setup_submodule() {
    local path=$1
    local repo_name=$2
    local branch=${3:-main}  # Default to main if no branch specified
...
# --- Handle HRM-specific submodules ---
echo "=== Setting up HRM Development Environment ==="

# Set up shared-ui with hrm branch
setup_submodule "app/shared-ui" "sm-workos-shared-ui" "hrm"

# Set up domain-apps (if needed for HRM)
...
echo "--- HRM Development environment setup complete! ---"
echo "Shared-UI is now using the 'hrm' branch." 