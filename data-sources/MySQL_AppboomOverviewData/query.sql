SELECT
  ...
  IFNULL(dl.downloads, 0) AS downloads,
  IFNULL(cc.custom_costs, 0) AS custom_costs
FROM (
    SELECT node_id, date FROM aa_node_dailystat WHERE date BETWEEN @startDate AND @endDate
    UNION
    SELECT node_id, date FROM aa_revenue_iap WHERE date BETWEEN @startDate AND @endDate
    UNION
    SELECT node_id, date FROM aa_revenue_cpc WHERE date BETWEEN @startDate AND @endDate
    UNION
    SELECT node_id, date FROM aa_spend WHERE date BETWEEN @startDate AND @endDate
) AS ds
...
) AS dl ON dl.node_id = ds.node_id AND dl.date = ds.date
...
WHERE ds.node_id IS NOT NULL AND n.node_id IS NOT NULL
