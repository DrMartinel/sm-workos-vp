# 📊 Cơ Chế Datasource Mới — Hệ Thống Reporting SmartmoveOS
...
Tham số	Mô tả
group_by	Danh sách dimension cần group
filter_by	JSON object chứa các filter, ví dụ: { country_code: ['VN'] }
metrics	Danh sách các metrics cần lấy (nếu không truyền sẽ lấy tất cả)
...
4.2 Filter & Date Range
	•	Inject vào WHERE của query.sql thông qua biến:
	•	@DS_START_DATE
	•	@DS_END_DATE
...
✅ Sắp tới, các trang phức tạp sẽ chuyển sang API riêng (/api/reports/...) để xử lý logic chart tại backend, giúp tối ưu hiệu năng.
...
8. 🚧 Định Hướng Nâng Cao (Tùy chọn)
	•	Tự động sinh chart từ schema datasource
	•	Hỗ trợ preload dữ liệu theo lịch (cron)
