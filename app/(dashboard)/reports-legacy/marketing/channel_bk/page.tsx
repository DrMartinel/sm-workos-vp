...
import { MarketingChannelPageSkeleton } from "@/components/overview/marketing-channel-page-skeleton"
...
function useAdjustCohortData(startDate?: string, endDate?: string) {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                console.time(`[FE] Total data fetch and process time for ${startDate}-${endDate}`);
                setLoading(true);
                setError(null);

                // Fetch current period data
                console.time(`[FE] API call for current period`);
                const fetchCurrentPeriod = fetch(`/api/data-source/BigQuery_AdjustCohortData?startDate=${startDate}&endDate=${endDate}`).then(res => {
                    ...
                    console.timeEnd(`[FE] API call for current period`);
                    return res.json();
                });
                ...
                console.time(`[FE] API call for previous period`);
                const fetchPreviousPeriod = fetch(`/api/data-source/BigQuery_AdjustCohortData?startDate=${prevStartDate}&endDate=${prevEndDate}`).then(res => {
                    ...
                    console.timeEnd(`[FE] API call for previous period`);
                    return res.json();
                });
                ...
                setLoading(false);
                console.timeEnd(`[FE] Total data fetch and process time for ${startDate}-${endDate}`);
            }
        }
        fetchData();
    }, [startDate, endDate]);
...
export default function ReportsPage() {
    ...
    console.time('[FE] chartData calculation');
    ...
    console.timeEnd('[FE] chartData calculation');
    ...
    console.time('[FE] postInstallMetrics calculation');
    ...
    console.timeEnd('[FE] postInstallMetrics calculation');
